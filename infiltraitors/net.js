/**
 * net.js — Supabase 网络通信层（渗透因子 · 联机协作版）
 * 负责房间 CRUD + Realtime 订阅；1 房主 + 1~4 成员，共 2~5 人。
 *
 * 用法：
 *   1. 在 HTML 中引入 supabase CDN：
 *      <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
 *   2. 本文件随 index_ol.html 加载（房间表与其余游戏共用，前缀隔离）
 */

// ════════════════════════════════════════════════════════════════
//  配置区（不同游戏用不同前缀，避免数据冲突）
// ════════════════════════════════════════════════════════════════
...
const NET_CONFIG = {
  ...
  ROOM_PREFIX: 'infil-ol-', // 渗透因子联机房间前缀
  MAX_PLAYERS: 5,           // 2~5 人协作
};
...
async function netJoinRoom(shortId, playerName) {
  ...
  if (room.status === 'finished') throw new Error('房间已解散');
  if (room.seats.length >= NET_CONFIG.MAX_PLAYERS) throw new Error('房间已满（最多 ' + NET_CONFIG.MAX_PLAYERS + ' 人）');

  const seats = [...room.seats];
  // 取最小空闲座位号：中途有人退出会留下空洞，直接取 length 会与在位玩家撞号
  let seatIndex = 0;
  while (seats.some(s => s.seatIndex === seatIndex)) seatIndex++;
